...
void swarm_setpoint(ros::Publisher& local_pos_pub, ros::Rate& rate, double offset_x, double offset_y, double offset_z, double min_height, bool heading) {
...
{
    ros::init(argc, argv, "swarm_heterogeneous");
    ros::NodeHandle nh;
    ...
    while(ros::ok()){
        if (std::abs(leader_pitching) >= 0.785398 || std::abs(leader_rolling) >= 0.785398) {
            take_down_flag = true;
        }       
        if (ros::ok() && uav1_current_state.connected) {
            if (uav0_current_state.mode == "AUTO.RTL" || uav0_current_state.mode == "AUTO.LAND" || take_down_flag == 1) {
                ...
                swarm_setpoint(uav1_pos_pub, rate, -5, -5, 10, 2, true);
                ros::spinOnce();
                rate.sleep();  
            }
        ...
        if (ros::ok() && uav2_current_state.connected) {
            if (uav0_current_state.mode == "AUTO.RTL" || uav0_current_state.mode == "AUTO.LAND" || take_down_flag == 1) {
                ...
                swarm_setpoint(uav2_pos_pub, rate,  5, -5, 10, 2, false);
                ros::spinOnce();
                rate.sleep();
            }
